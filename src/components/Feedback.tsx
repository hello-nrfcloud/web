export const Feedback = () => (
	<section
		style={{
			backgroundColor: 'var(--color-nordic-dark-grey)',
		}}
		class="pt-4 pb-4 text-white"
	>
		<div class="container mt-4">
			<div class="row">
				<div class="col-6">
					<h2>Got feedback? Get a t-shirt!</h2>
					<p>
						We'd love to hear your feedback. We are interested to learn what you
						specifically liked, or didn't like, and maybe what's missing!
					</p>
					<p>
						In any case, we are very grateful for any feedback that you have for
						us!
					</p>
					<p>
						Every month, we'll select 5 new feeback posts that we liked best on{' '}
						{'{DevZone'} that are tagged with{' '}
						<a
							href="https://devzone.nordicsemi.com/f/nordic-q-a/tags/hello-nrfcloud"
							target="_blank"
						>
							<code>hello-nrfcloud</code>
						</a>{' '}
						and give out a little present. It could be a T-Shirt; or something
						else and we may at any time decide to not hand out gifts, just so
						you know.
					</p>
					<p>Regardless, we will read all your feedback!</p>
					<p>
						<a
							href="https://devzone.nordicsemi.com/support/add"
							target="_blank"
						>
							Submit your feedback now!
						</a>
					</p>
				</div>

				<div class="col-3">
					<p>
						<img
							src="/static/images/t-shirt.webp"
							alt="A person wearing a black t-shirt that reads: hello, I connected a Nordic DK to the Internet before I had my first coffee!"
							class="img-fluid"
						/>
					</p>
					<p>
						<small>Picture used solely for illustrative purposes.</small>
						<br />
						<small>
							Photo by <a href="https://unsplash.com/es/@sadswim">ian dooley</a>{' '}
							on <a href="https://unsplash.com/photos/kkj9iKxsdhY">Unsplash</a>.
						</small>
					</p>
				</div>
			</div>
		</div>
	</section>
)
